...
function DropIndicator({ position, parentId, index }: DropIndicatorProps) {
  ...
  const [{ isOver, canDrop }, drop] = useDrop(() => ({
    ...
    drop: (item: any, monitor) => {
      console.log("DropIndicator drop triggered:", {
        item,
        position,
        parentId,
        index,
...
export function RenderElement({
  ...
  const isSelected =
    state.selectedElement === element.id && state.editingMode === "editing";
  const canHaveChildren =
    CONTAINER_ELEMENTS.includes(element.tag) && element.type !== "custom-html";
  ...
  console.log(
    `👁️ Element ${element.id} (${element.content || element.tag}) visibility:`,
    {
      interactionState,
      isVisible,
  ...
  const handleDuplicate = (e: React.MouseEvent) => {
    ...
    const newInteractions = element.interactions
      ? JSON.parse(JSON.stringify(element.interactions))
      : undefined;
      ...
      <div className="relative">
        ...
        <ElementComponent {...elementProps}>
          ...
          {element.children.length === 0 && (
            <DropIndicator position="top" parentId={element.id} index={0} />
          )}
